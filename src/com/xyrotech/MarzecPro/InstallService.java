...
public class InstallService extends Service {
    ...
    class InstallThread extends Thread {
        ...
        public void writeCommand(OutputStream os, String command) throws Exception {
            Log.i(MarzecPro.TAG, "writeCommand: " + command);
        ...
        public void run() {
            ...
            try {
                ...
                Log.i(MarzecPro.TAG, "exec: " + suCmd);
                ...
                String command = "./create-marzec-setup.sh " + stdArgs;
                command += "\\\n&& " + "./stop-marzec.sh " + stdArgs;
                command += "\\\n&& " + "./unmounted-install-tweaks.sh " + stdArgs;
                command += "\\\n&& " + "./start-marzec.sh " + stdArgs;
                command += "\\\n&& " + NativeHelper.app_bin + "/chroot " + NativeHelper.mnt
                        + " " + NativeHelper.app_bin + "/complete-marzec-installation.sh "
                        + stdArgs;
                writeCommand(os, "cd " + NativeHelper.app_bin.getAbsolutePath());
                //navigate to install_adds.sh if this fails
                writeCommand(os, "./install_adds.sh");
                writeCommand(os, command);
                // Avoid keeping the resource mounted because of some failure
                writeCommand(os, "./stop-marzec.sh " + stdArgs);
