...
if [ ! -e $mnt ]; then
    echo "Your MarzecPro setup is missing mountpoint."
    echo "    mkdir $mnt"
    mkdir $mnt || exit
    chmod 755 $mnt
    exit
...
if [ ! -d $sdcard ]; then
    echo "Your MarzecPro setup is missing sdcard: $sdcard"
    exit
fi
...
if [ x"$install_on_internal_storage" = xno ] && [ ! -b $loopdev ]; then
    echo "Your MarzecPro setup is missing loopdev: $loopdev"
    exit
fi
...
if [ x"$install_on_internal_storage" = xno ]; then
    ...
    # use system or attacks fsck to check Debian partition
    echo ""
    find_and_run_fsck
...
if [ ! -e /debian/shell ]; then
    echo "installing '/marzec/shell' for easy way to get to chroot from term"
